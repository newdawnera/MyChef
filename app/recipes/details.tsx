...
export default function RecipeDetailsScreen() {
    ...
    const pulse = Animated.loop(
      Animated.sequence([
        Animated.timing(pulseAnim, {
          toValue: 1.15, // Scale up (Increase)
          duration: 1000,
          useNativeDriver: true,
        }),
        Animated.timing(pulseAnim, {
          toValue: 1, // Scale down (Reduce)
          duration: 1000,
          useNativeDriver: true,
        }),
      ])
  ...
  const handleMarkAsCooked = async () => {
    ...
    try {
      ...
      console.log("✅ Recipe cooked status updated successfully");
    } catch (error) {
      // Revert on error
      setIsCooked(!newCookedState);
  ...
  // Improved tab change with smooth spring animation
  const handleTabChange = (tab: "ingredients" | "instructions") => {
    ...
    Animated.parallel([
      ...
      // Fade in and spring new content
      Animated.parallel([
        Animated.timing(fadeAnim, {
          toValue: 1,
          duration: 200,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <Animated.View
        style={{
          position: "absolute",
          bottom: bottom + 90 || 100,
          right: 20,
          zIndex: 10,
          transform: [{ scale: pulseAnim }], // Apply the pulsing scale
        }}
      >
        <TouchableOpacity
          onPress={() => {
            setShowAIChat(true);
            // We removed the old 'aiBtnScale' bounce here because the pulse handles the motion now
          }}
          style={{
            width: 60,
            height: 60,
            ...
            shadowOpacity: 1, // Stronger shadow
            shadowRadius: 18, // Wider spread
